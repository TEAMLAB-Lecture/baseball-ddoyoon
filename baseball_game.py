...
def is_duplicated_number(three_digit:str) -> bool:
    ...
    # ===Modify codes below=============
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당
    num_set = set([int(d) for d in str(three_digit)])
    result = True if len(num_set) < 3 else False
    # ==================================
    return result
...
def get_not_duplicated_three_digit_number() -> int:
    ...
    while not done:
        ...
        if not is_duplicated_number(str(generated_number)):
            done = True
    result = generated_number
    # ==================================
    return result
...
def get_strikes_or_ball(user_input_number: str, random_number: str):
    ...
    for i in range(len(user_input_number)):
        user_digit = user_input_number[i]
        if user_digit == random_number[i]:
            strike += 1
        elif user_digit in random_number:
            ball += 1
    result = [strike, ball]
    # ==================================
    return result
...
def is_yes(one_more_input: str) -> bool:
    ...
    # ===Modify codes below=============
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당
    response = one_more_input.upper()
    result = True if response == "Y" or response == "YES" else False
    # ==================================
    return result
...
def is_no(one_more_input: str) -> bool:
    ...
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당
    response = one_more_input.upper()
    result = True if response == "N" or response == "NO" else False
    # ==================================
    return result
